...
var cmd = 'which hyperion-v4l2';
var hyperion_v4l2_cmd;
var hyperion_pid;
...
adapter.on('stateChange', function (id, state) {
    if (!state.ack) {
        ...
        } else if (id_arr[3] === 'start') {
		
		if (!hyperion_pid){
			hyperion_pid = spawn(hyperion_v4l2_cmd);
			adapter.log.info(JSON.stringify(hyperion_pid));
		}
		adapter.log.info("Starting hyperion-v4l2 thread...");
	
	
	
	} else if (id_arr[3] === 'stop') {
		if (hyperion_pid){
			hyperion_pid.kill('SIGHUP');
			adapter.log.info("Stopping hyperion-v4l2 thread...");
		}
...
function main() {
    ...
    if (os.type() == 'Linux'){
        exec(cmd, function(error, stdout, stderr) {
             if (stdout !== ''){
		 hyperion_v4l2_cmd = stdout.trim();
		 var myls = ls('/dev/video*');
		 if (myls.length >= 1){
				//ok were on linux, having hyperion-v4l2 and at least on /dev/videoX device. so let start over
				var myv4l2ctrlobj = {type: 'channel', common: {name: 'hyperion-v4l2'}, native:{id: 'hyperion-v4l2'}};
				adapter.setObject('hyperion-v4l2' , myv4l2ctrlobj);
				var mystartv4l2 = {type: 'state', common: {role: 'button', type: 'state', name: 'start_hyperion-v4l2'}, native:{id: 'hyperion-v4l2.start'}};
				adapter.setObject('hyperion-v4l2.start',mystartv4l2);
				var mystopv4l2 = {type: 'state', common: {role: 'button', type: 'state', name: 'stop_hyperion-v4l2'}, native:{id: 'hyperion-v4l2.stop'}};
				adapter.setObject('hyperion-v4l2.stop',mystopv4l2);
				var mypriov4l2 = {type: 'state', common: {role: 'number', type: 'state', name: 'prio_hyperion-v4l2', def: 100}, native:{id: 'hyperion-v4l2.prio'}};
				adapter.setObject('hyperion-v4l2.prio',mypriov4l2);
				//adapter.setState(mypriov4l2, 100, true);
				var mydevv4l2 = {type: 'state', common: {role: 'number', type: 'state', name: 'video_device_hyperion-v4l2', def: 0}, native:{id: 'hyperion-v4l2.dev'}};
				var states = {};
				var i = 0;
				for (var file of ls('/dev/video*')) {
					states[i] = '/dev/' + file.name;
					adapter.log.debug("Found video4linux2 device at: " + file.name + "for use with hyperion-v4l2!")
				}
				mydevv4l2.common.states = states;
				adapter.setObject('hyperion-v4l2.dev',mydevv4l2);
				//adapter.setState(mydevv4l2, 0, true);
				adapter.log.debug("Found hyperion-v4l2 binary at: " + hyperion_v4l2_cmd);    
			};
		};
	});
     };
